...
public:
    ...
    : Node("buff_detector_node", options)
    {
        ...
        detector_config_.openvino_device =
            this->get_parameter("openvino_device").as_string();
...
private:
    ...
    void processFrame(cv::Mat frame, const rclcpp::Time& frame_stamp)
    {
        ...
        {   
            ...
            {   
                cv::Mat debug_frame = detector_->debug_frame_.clone();
                cv::circle(
                    debug_frame,
                    cv::Point(debug_frame.cols / 2, debug_frame.rows / 2),
                    4,
                    cv::Scalar(0, 255, 0),
                    2);
