...
bool BuffDetector::detect_by_yolo(cv::Mat& frame)
{
    ...
    target_blades_.push_back(FanBlade{
        rectToBBox(output.fan_box),
        FanBladeState::target,
        -1
    });

    if (buff_type_ == BuffType::big_buff) {
        const int secondary_index = findSecondaryCandidateIndex(
            output, config_.mode_judge_far_distance_px, true);
...
bool BuffDetector::preprocess_image(cv::Mat& frame)
{
    ...
    preprocessed_frame_ = frame.clone(); // 保存预处理后的图像供调试使用
...
bool BuffDetector::update_fan_blades(cv::Mat& image)
{
    ...
    roi_frame_ = image.clone(); // 保存ROI图像供调试使用
