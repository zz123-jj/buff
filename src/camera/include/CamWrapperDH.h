...
class DHCamera : public Camera {
   ...
   private:
    ...
    Camera::FrameCallback frame_callback_;
    int64_t timestamp_tick_frequency_{0};

   public:
    DHCamera(std::string sn, int roi_w, int roi_h);  // constructor, p_img is a pointer towards a
    // 640*640 8uc3 Mat type
    ~DHCamera();

    bool init(int roi_x, int roi_y, int roi_w, int roi_h, float exposure, float gain,
              float FPS, int nBinning, int rotate_type = -1) final;  // camInit camera lib and do settings, be called firstly
    ...
    void set_frame_callback(FrameCallback cb) final;
    uint64_t tick_frequency() const final { return static_cast<uint64_t>(timestamp_tick_frequency_); }
};
