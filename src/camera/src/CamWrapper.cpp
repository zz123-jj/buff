...
bool DHCamera::init(int roi_x, int roi_y, int roi_w, int roi_h, float exposure, float gain,
    ...
    if (status != GX_STATUS_SUCCESS) {
        LOG(ERROR) << "GXInitLib failed. status=" << status;
        return false;
    }
    ...
    if (status != GX_STATUS_SUCCESS) {
        LOG(ERROR) << "GXUpdateDeviceList failed. status=" << status;
        return false;
    }
    ...
    if (nDeviceNum >= 1) {
        ...
        if (status != GX_STATUS_SUCCESS) {
            LOG(ERROR) << "GXGetAllDeviceBaseInfo failed. status=" << status;
            return false;
        }
        ...
        if (status != GX_STATUS_SUCCESS) {
            LOG(ERROR) << "GXOpenDevice failed. status=" << status;
            return false;
        }
        device_opened = true;
        ...
        if (g_frameData.pImgBuf == NULL || g_pRaw8Buffer == NULL) {
            LOG(ERROR) << "failed to allocate camera frame buffers";
            return false;
        }
...
bool DHCamera::start() {
    if (init_success) {
        ...
        status = GXRegisterCaptureCallback(g_hDevice, this, OnFrameCallbackFun);
        if (status != GX_STATUS_SUCCESS) {
            LOG(ERROR) << "GXRegisterCaptureCallback failed. status=" << status;
            return false;
        }
        status = GXSendCommand(g_hDevice, GX_COMMAND_ACQUISITION_START);
        if (status != GX_STATUS_SUCCESS) {
            LOG(ERROR) << "GX_COMMAND_ACQUISITION_START failed. status=" << status;
